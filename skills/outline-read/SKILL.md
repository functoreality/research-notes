...
级别 1 和级别 3 的区别：级别 1 只在目标行的**紧邻父级**展开兄弟节点（「这一节下还有哪些子话题」），但不会展开更高层级（祖父、曾祖父）的兄弟。级别 3 则在所有层级都展开。比如在深层嵌套的目标上，级别 1 可能从 13 行增加到 ~19 行，而级别 3 可能暴增到 60+ 行。级别 2 在级别 1 的基础上额外展示全部顶层标题，提供文件级方向感。
