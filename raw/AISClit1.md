...
* `ISMO-2008.05730`: #active_learning, #surrogate, #optimal_control, #PDE, (#open_source)
	...
	* > date: 2020-11-18 讨论
		...
		* 证明应该好好准备，我们会关心文章是否回答了 DL 的一些根本性的问题，如 sample, 收敛性等
			* 本文直接回避了这些点，没有 non-trivial 部分；主要卖点不在这里，在算法
...
* `BIP-ADNN-1911.08926`: #surrogate, #BIP (#Bayesian_inverse_problem), #active_learning, #MCMC, #elliptic_PDE_inverse_problem (#in_experiment)
	...
	* 实验：（参考了引文）从观测到的 $u$（给定 grid 上的取值；带噪声）恢复 $\kappa$
		...
		* sec4:2 迭代次数等参数设定，surrogate 至多只更新 50 次（回忆只在误差大于给定值时才更新）
		...
		> (?) p15:1, table1 和纯先验的做法比较，没有使用（大致）相同的训练样本总数？并且，纯先验的做法使用的网络没有 $\mathcal{NN}^\text{H}$ 部分，网络拟合能力本来就有差距？这种比较不公平！table3 关于邻域采样数 $Q$ 的实验同理
	...
	* > date: 2021-01-20 讨论
		* 结果偏弱，这方面不如 ISMO 那么 convincing，也没有单独的理论分析
		* MCMC 问题其实用 normalizing flow 生成模型也可以做，同样可以处理后验分布归一化系数未知的情形，并且可以处理更加复杂的先验分布（这篇文章使用的先验太简单了），如图像的先验分布维数高的多，并且不知道具体表达式只能从数据中推断
			* forward model 需要简化版本，在这里是一致的
		* 另有 SDE sampling（文章见群里）；MCMC 通常要求 $t\to\infty$，而这一做法则很快收敛，比较神奇，可以生成 $1024\times 1024$（高维）的高分辨率图像
			> `BIP-ADNN-1911.08926`
