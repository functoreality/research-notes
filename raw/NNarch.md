	...
	* 引入（显式/硬）先验知识：对称性，等变性，物理知识；{nca99w}
		...
		* 等变性 equivariance；{n3h983}
			...
			* （严格）等变性架构设计
				...
				* 各种群下有 equivariance 性质的映射有特定形式 `2022-03-30`(AISCmeet2)，可在架构设计中体现；{n23g9c}
					* 架构设计理论框架，((_n7mj31))用 steerable kernel 给出等变网络的一般形式，包括标量、向量的变换
				* 网格旋转等变性，一套卷积核旋转多个角度同时作用于输入，((_p78a72))可用于交错网格情形
				* 其他涉及等变性参考材料：`2022-08-03`(AISCmeet2) 群等变 CNN，steerable G-conv，参考链接
				* 特殊情形：算子等变性，其输入为函数，对函数的群作用通过对其定义域的作用给出；{n23h00}
					* 对 PDO 已有相关构造方式的研究((n23g87))，包括系数自适应依赖于空间位置的情形
				* 分子空间构型预测要求平移、旋转不变性，((n2me84))扩散生成模型中形如 GNN 的网络自动满足；{n2me8g}
				* Transformer（ViT）也可引入旋转等群等变性质((n3ha70))
				* `INSP-Net-2210.08772` 作用于 INR 的 NO，表达为可学微分算子作用，自动平移不变（等变？）、设计微分算子 ansatz 可旋转不变
					* 基于（物理空间）PDE 的网络架构设计应都能以类似方式保证平移、旋转不变
				* 频域等变性，((n7jm27))G-FNO 架构涉及 在频域的 activation 值用其群变换结果增强；{n7jm2c}
