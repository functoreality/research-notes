	...
	* 引入（显式/硬）先验知识：对称性，等变性，物理知识；{nca99w}
		...
		* 等变性 equivariance；{n3h983}
			* 对应的群作用：平移，旋转；群包括连续版本与离散版本
				* PDE 数据增强，((n8ve45))可根据特定方程形式用更复杂的 Lie 群作用；{n8ve48}
			* （严格）等变性架构设计
				* 区分架构所表达的映射类型
					* 输入输出—散点数据，输入输出为相同散点集上的数据；分子领域常涉及
					* 输入输出—物理场，对函数的群作用通过对其定义域的作用给出，算子等变性；{n23h00}
...
* 引入随机性（前传），有时可提高稳定性；{n37n39}
		...
		* （一般意义上的）activation 相关：{ncab8m}
			...
			* normalization 正则化；{n9gg5k}
				...
				* 统计量：均值+方差，RMSNorm 仅方差
					...
					* 输出再调整统计量：固定（均值 0 方差 1），逐分量可学（默认情形），外部输入调制
				...
				* NO 或需去掉 LayerNorm：（线性注意力 Transformer 架构）若训练数据未归一化，则考虑去掉层间 layernorm 以使尺度信息可跨层传播((n9gg54))
	...
	* 输入：一般的((n31n1z))数据增强 方式均可用，包括加噪声
		* MAE，((n37m6z))导师认为 mask out 属于一种特殊的噪声；推断时也有噪声 ensemble 做法
	* 用途—训练，前传引入随机性的目的
			...
			* 预训练的 mask out 策略，MAE 等；{n37m7m}
				...
				* 理解方式
					* 正则化：`2022-01-28`(CSImeet2) 导师认为 MAE 做法不应只理解为无监督学习，它可视为加某种噪声（mask 视为特殊信息污染方案）引入的正则化；{n37m6z}
						* 推断时也可多次随机 mask out 取 ensemble；{n8mn6i}
							* 一般框架：((p8nf0w))提预测可靠性-网络前传取 ensemble
						* 有工作将 MAE 作为数据增强策略((_n37m82))；{n37m8g}
						* 相关框架：((n37g9c))优化-噪声引入方式，((n31n1z))数据增强，((n4sk0m))噪声相当于正则化
					* `2202.03670` 从 kernel learning, low rank approximation 等角度解释 MAE
				* 不以重建 mask 部分为训练目标：((_o4tl74))FLIP 在 CLIP 中输入的图文模态均引入 mask
