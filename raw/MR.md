...
* `1812.01522`: #phase_transition, 
	...
	* 训练一个 NN，输入 $L\times L$ 网格上的构象，输出温度预测（分类问题，设定若干温度刻度）
		> 作者应该不那么熟悉 DL，我猜是 CNN 但文中没说，并且也没强调为何用分类而不用回归网络
...
* `ML2phy%` 学习出物理模型：{n3bh3b}
	...
	* 分类比较标准
		...
		* 输入的形式，除有限维外，还可为无穷维函数；此时可涉及间接观测的导数项，获得方式：{n23j97}
			* 相关，其他求导场景：((n2o879))对主动构造的场求导（用于算正问题），((n32e95))一般映射求导
			* 数值微分（通常需网格输入，如 PDE-Net；可能性：散点用图上的梯度、散度等算子）
			* NN 作中介先拟合数据；{n79i5u}
				* 对 PDE 允许不规则观测点 `2021-11-03`(lectures)“2005.03448”
			* 若动力系统若网格分辨率低/观测带噪声：架构有约束的 NO 可描述高分辨率空间中动力学，令其拟合观测；高分辨率空间可数值微分
				* 从而每步的高分辨率重建都（隐式地）用了所有时间步的低分辨率数据
				* 所恢复的高分辨率数据可加正则化约束防振荡
				* eg. `PeRCNN-2106.04781`
					* 其中还训了预测器，据低分辨率初值生成高分辨率初值，但感觉只在数据多初值时才重要
			* 均匀密网格、观测带噪声：((_n79i3z))SDD 先对数据卷积以光滑化，再用中心 5 点差分；高阶导数靠复合作用（再卷积、再差分）获得；{n79i92}
	...
	* 方程中未知项给出 NN 表示；{pcfj4q}
		...
		* 相关框架：((pcfj5p))方程项由 NN 表示，此处依据为观测数据，另有依据底层原理从头算模拟数据的
	* 提取对系统性质的描述（定性为主）
		* `PhysRevE.103.033303` 为找守恒量表达式，先找了保对称性的变换，相当于找对称性
		* SciNet 据说从数据中学出了 Kepler 三定律（是否算模式识别？）
		* `AgentNet-2001.02539` 复杂系统底层机理提取，用 GNN 注意力表达主体间相互作用方式；{ncaa6c}
			* 相关：((n3gm7a))注意力机制
		* `2205.10965` 根据动力学（可多体）数据，从极限环中划分不同时间尺度部分（通过符号回归后找拟合差的区段），可分析该划分如何被系统改变（如引入另一种动力学的分量）影响
		* `NEEP-2003.04166` 学映射以给出马氏链两状态转移的熵产生量，只需用时序数据无监督训练，`CNEEP-2106.15108` 在用视频数据时进一步识别熵产生位置
		* `NNPhD-2106.00026` 近似守恒系统中提取非守恒部分
		* `Huang2022ExtractingCE`（非 ML）观测的系统带源项，希望提取无源项时的系统 Lagrangian 形式（包括符号表达）
