...
* `ML2phy%` 学习出物理模型：{n3bh3b}
	...
	* 提取对系统性质的描述（定性为主）
		...
		* 空间异质性（强度分布）检测
			* 新物理的活跃作用区域：((_q7am8x))已知机制预测结果与数据有出入，训新模块拟合该差异，其后可检验该新模块的活跃区域
				* 原文方法基于((q7an1i))动力学部分项已知-算子分裂学未知项
		...
		* 熵产量检测：`NEEP-2003.04166` 学映射以给出马氏链两状态转移的熵产生量，只需用时序数据无监督训练
			* `CNEEP-2106.15108` 在用视频数据时进一步识别熵产生位置；{q7am94}
