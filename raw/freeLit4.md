...
* （备用）GLM5.2 放弃 GRPO 改回 PPO 相关讨论
	* [2026-07-08](https://www.zhihu.com/question/2052108642686706557/answer/2052367150749427260)
	...
	> 回顾下GRPO当时出来时后大家公认的赖以成立的三个前提，第一个是同题可比较，第二个是信号可验证，第三个是组内能降噪。
		> 这三个前提在目前的长程任务中，不管是算法角度还是工程角度，都有一定程度的被动摇。
	> 回到业界具体的落地上，不管是智谱、minimax还是创造GRPO算法的DeepSeek，他们对于长程编程的训练是在不同的层面解决问题。
		> 智谱是直接把critc网络请回来，重新衡量token级的优势。
		> Minimax则是设计了一整套新的RL算法和对应的 Infra，CISPO算法、再加复合奖励，包括过程、时间、结果，再加Forge调度。
		> DeepSeek则是通过GRPO训了专家模型，使用一种叫On-Policy Distillation的方式让模型在自己探索的同时，专家模型来提供监督信号。
		> 这三家的解法掰开来看，本质上还是承认了GRPO在长程任务中的局限性。
