...
* 2406.08473 NO 预训练策略综述、实验比较
	...
	* sec3.2 预训练策略
		* 3.2.1 CV 中预训练策略：
			* binary 预训练（判断数据是否被打乱时间顺序）{_q8bf63}
			* 对各帧进行正确排序（$\#S_n$-路分类任务）
			* 另有空间 shuffle 策略，经验上表现不佳，故本文实验中省略；{_o73j9m}
...
* `OneShotNO-2104.05512`: #NO, #few-shot
	...
	* 方法：1. 在网格上逐点 $U[-1,1]$ 生成 $f_T$
		> 没有对应的连续极限
	* 2. 学习邻域预测算子 $\tilde G:(u|\partial U(x),f|U(x))\mapsto u(x)$ or $(u|U_0(x),f(x))\mapsto u(x)$（限制在网格上）{_q8g70x}
