...
* `SFNO-2306.03838` 球面上定义的 FNO，利用球谐函数
	...
	* 2023-08-08 CSImeet，导师：这种做法我们无需过多关注，虽然数学框架漂亮，但不太利于工程实现，工程上本来也有更好的解决方案；之前和别人的共识是能变换到规则区域就用针对规则区域的网络
...
* `FNO-2010.08895`: #parameteric_PDE (#grid-independent), #NO
	...
	* eqn(5) $\mathcal{K}$ 算子形式
		* （日后补充的简短版本）$\mathcal{K}v=iFT(R(FT(v)))$，$R$ 直接参化为 $s_1\times\cdots\times s_d\times d_v\times d_v$ 张量
		...
		<!-- * 矩阵卷积核在 FT 下为矩阵向量乘法，故只参数化频域的卷积核 -->
	...
	* 导师：与 `flowMap-1910.06948` 想法相似度高但没有引（我自己写文章可以引）
		* 2021-10-29 CSI讨论，据说实验效果不如 Galerkin Transformer，时间迭代外插不太好
	* 其他相关工作：
